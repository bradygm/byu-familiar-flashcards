...
async function courseView(courseId) {
  const [course, cards, candidates, stats] = await Promise.all([api(`/courses/${courseId}`), api(`/courses/${courseId}/cards?sort=last`), api(`/courses/${courseId}/candidates`), api(`/courses/${courseId}/stats`)]);
  currentCourse = course;
  const accuracy = stats.reviews ? Math.round(stats.right_count / stats.reviews * 100) : 0;
  setView(`<a class="back" href="#/">← All courses</a><section class="section-head" style="margin-top:25px"><div><div class="eyebrow">${esc(course.source_filename)}</div><h1>${esc(course.title)}</h1><p>${cards.length} approved cards · ${candidates.length} waiting for review</p></div><div class="actions"><button class="button secondary" id="review-candidates">Review imports</button><button class="button secondary" id="add-card">Add person</button><button class="button" id="start-study">Start session</button></div></section><section class="stats" aria-label="Course statistics"><article class="stat panel"><strong>${stats.session_count}</strong><span>sessions</span></article><article class="stat panel"><strong>${stats.reviews}</strong><span>answers</span></article><article class="stat panel"><strong>${accuracy}%</strong><span>accuracy</span></article><article class="stat panel"><strong>${stats.wrong_count}</strong><span>misses</span></article></section><div class="toolbar"><input class="search" id="search" placeholder="Search people" aria-label="Search people"><select class="select" id="sort" aria-label="Sort roster"><option value="last">Last name</option><option value="first">First name</option></select></div><div id="roster"></div>`);
  const roster = document.querySelector('#roster');
  function renderRoster(items) { roster.innerHTML = items.length ? `<div class="roster">${items.map(card => `<article class="person panel">${portrait(card)}<h2>${esc(card.first_name)} ${esc(card.last_name)}</h2><div class="fine">${card.seen_count ? `${card.right_count}/${card.seen_count} correct` : 'New to you'}</div><span class="tag">confidence ${Math.max(0, card.confidence).toFixed(1)}</span></article>`).join('')}</div>` : `<div class="empty"><h2>No approved cards yet.</h2><p>Review the imported candidates, or add cards after your first successful PDF import.</p></div>`; }
  renderRoster(cards);
...
function setupView(course, count) {
  let mode = 'adaptive';
  setView(`<a class="back" href="${courseLink(course)}" id="setup-back">← ${esc(course.title)}</a><section class="setup"><div class="eyebrow">Study setup</div><h1>What feels useful today?</h1><p class="fine">Every card is available whenever you are. Adaptive review simply makes a varied, helpful choice.</p><div class="mode-grid"><button class="mode selected" data-mode="adaptive"><h2>Adaptive review</h2><p>Prioritizes new and uncertain people, with a few familiar faces mixed in.</p></button><button class="mode" data-mode="all"><h2>All cards</h2><p>See every approved person once, in a fresh random order.</p></button></div><label class="range">Adaptive session length: <strong id="length-label">15</strong><input id="length" type="range" min="5" max="${Math.max(5, Math.min(50, count))}" value="${Math.min(15,count)}"></label><div class="actions"><button class="button" id="begin">Begin studying</button></div><div id="setup-notice"></div></section>`);
  document.querySelector('#setup-back').addEventListener('click', event => { event.preventDefault(); courseView(course.id); });
  document.querySelectorAll('[data-mode]').forEach(button => button.addEventListener('click', () => { mode = button.dataset.mode; document.querySelectorAll('[data-mode]').forEach(item => item.classList.toggle('selected', item === button)); document.querySelector('.range').classList.toggle('hidden', mode === 'all'); }));
  document.querySelector('#length').addEventListener('input', e => document.querySelector('#length-label').textContent = e.target.value);
  document.querySelector('#begin').addEventListener('click', async () => { try { const result = await api(`/courses/${course.id}/sessions`, {method:'POST', body:JSON.stringify({mode,limit:+document.querySelector('#length').value})}); study = {...result, index:0, revealed:false}; studyView(); } catch(error) { document.querySelector('#setup-notice').innerHTML = notice(error.message); } });
}

function studyView() {
  const card = study.cards[study.index];
  if (!card) return completeStudy();
  const complete = Math.round((study.index / study.cards.length) * 100);
  setView(`<div class="study-wrap"><div class="session-meta"><span>${study.mode === 'all' ? 'All cards' : 'Adaptive review'}</span><span>${study.index + 1} / ${study.cards.length}</span></div><div class="study-card" id="flashcard" role="button" tabindex="0" aria-label="Flip card">${portrait(card)}${study.revealed ? `<div class="answer"><div class="eyebrow">The answer</div><div class="name">${esc(card.first_name)} ${esc(card.last_name)}</div>${card.facts.length ? `<ul>${card.facts.map(fact=>`<li>${esc(fact)}</li>`).join('')}</ul>` : '<p>No additional facts yet.</p>'}</div>` : `<div><div class="eyebrow">Who is this?</div><h1>Can you name them?</h1><p>Flip when you have an answer in mind.</p><span class="key">Space</span> to flip</div>`}</div><div class="study-actions">${study.revealed ? `<button class="button danger" id="wrong">Wrong <span class="key">W</span></button><button class="button" id="right">Right <span class="key">R</span></button>` : `<button class="button" id="flip">Flip card <span class="key">Space</span></button>`}</div><div class="fine" style="margin-top:18px">${complete}% complete · <span class="key">Esc</span> to end session</div></div>`);
...
async function score(result) { const card = study.cards[study.index]; await api(`/sessions/${study.id}/reviews`, {method:'POST',body:JSON.stringify({card_id:card.id,result})}); study.index += 1; study.revealed = false; studyView(); }
...
document.addEventListener('keydown', event => { if (!study || ['INPUT','SELECT','TEXTAREA'].includes(document.activeElement.tagName)) return; if ((event.key === ' ' || event.key === 'Enter') && !study.revealed) { event.preventDefault(); study.revealed = true; studyView(); } if (study.revealed && event.key.toLowerCase() === 'r') score('right'); if (study.revealed && event.key.toLowerCase() === 'w') score('wrong'); if (event.key === 'Escape') completeStudy(); });
