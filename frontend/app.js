...
function readinessTrend(sessions) {
  ...
  const dots = sessions.map((session, index) => `<circle cx="${x(index)}" cy="${y(session.readiness)}" r="3"><title>${shortDate(session.ended_at)} · ${session.readiness}% course readiness</title></circle>`).join('');
  ...
  return `<div class="trend-chart"><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Course readiness across the last ${sessions.length} completed sessions">${guides}<polyline points="${points}"></polyline>${dots}</svg><div class="trend-foot"><span>${shortDate(sessions[0].ended_at)}</span><span>Last ${sessions.length} sessions</span><span>${shortDate(sessions[sessions.length - 1].ended_at)}</span></div></div>`;
}
...
async function courseView(courseId) {
  ...
  setView(`<a class="back" href="#/">← All courses</a><section class="section-head" style="margin-top:25px"><div><div class="eyebrow">${esc(course.source_filename)}</div><h1>${esc(course.title)}</h1><p>${cards.length} approved cards · ${candidates.length} waiting for review</p></div><div class="actions"><button class="button secondary" id="review-candidates">Review imports</button><button class="button secondary" id="add-card">Add person</button><button class="button" id="start-study">Start session</button></div></section><section class="stats" aria-label="Course statistics"><article class="stat panel"><strong>${stats.session_count}</strong><span>sessions</span></article><article class="stat panel"><strong>${stats.reviews}</strong><span>answers</span></article><article class="stat panel"><strong>${stats.readiness}%</strong><span>ready today</span></article><article class="stat panel"><strong>${stats.wrong_count}</strong><span>misses</span></article></section>${learningPulse(stats)}<div class="toolbar"><input class="search" id="search" placeholder="Search people" aria-label="Search people"><select class="select" id="sort" aria-label="Sort roster"><option value="first">First name</option><option value="last">Last name</option><option value="confidence">Predicted recall (low first)</option></select></div><div id="roster"></div>`);
