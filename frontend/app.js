...
function studyView() {
  ...
  setView(`<div class="study-wrap"><div class="session-meta"><span>${sessionTitle}</span><span>${position}</span></div><div class="study-card" id="flashcard" role="button" tabindex="0" aria-label="Flip card">${portrait(card)}${study.revealed ? `<div class="answer"><div class="eyebrow">The answer</div><div class="name">${esc(card.first_name)} ${esc(card.last_name)}</div>${card.facts.length ? `<ul>${card.facts.map(fact=>`<li>${esc(fact)}</li>`).join('')}</ul>` : '<p>No additional facts yet.</p>'}</div>` : `<div><div class="eyebrow">Who is this?</div><h1>Can you name them?</h1><p>Flip when you have an answer in mind.</p><span class="key">Space</span> to flip</div>`}</div><div class="study-actions">${study.revealed ? `<button class="button danger" id="wrong">Wrong <span class="key">W</span></button><button class="button" id="right">Right <span class="key">R</span></button>` : `<button class="button" id="flip">Flip card <span class="key">Space</span></button>`}</div><div class="fine" style="margin-top:18px">${complete}% complete · <span class="key">Esc</span> to end session</div></div>`);
